"""
taylor/taylor_tokenizer.py

Vocabulary and tokenizer for the Taylor expansion seq2seq task.

This is separate from data/tokenizer.py because the Taylor task needs:
  - Rational number tokens (1/2, 1/6, 1/24, etc.) as coefficients
  - A "neg" unary token for negation in Mul nodes
  - A generic "pow" binary token for arbitrary exponents
  - "log" operator (not present in the histogram tokenizer)
  - No <CONST> token (Taylor coefficients are exact, not predicted)

The design mirrors data/tokenizer.py (same encode/decode/wrap interface)
so the rest of the codebase feels consistent.
"""
...
# Integer tokens: covers polynomial coefficients and expansion points
...
class TaylorTokenizer:
    """
    Bidirectional mapping between token strings and integer IDs.

    Interface is identical to data/tokenizer.Tokenizer so both can be
    used interchangeably in shared utilities (AverageMeter, etc.).
    """
